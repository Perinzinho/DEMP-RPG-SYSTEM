...
function StatRow({ label, value, onChange, editable = true }: StatRowProps) {
  ...
  return (
    <div className="relative flex items-center gap-2 border-b border-[rgba(47,86,99,0.3)] py-1.5 text-[13px]">
      ...
      <button
        className="flex cursor-pointer items-center rounded border border-[#5A6056] p-1 text-sm text-[#6E97A4] hover:border-[#1a6fb5] hover:text-[#1a6fb5]"
        onClick={handleRoll}
      ...
      {rollResult && (
        <span
          className={cn(
            'absolute -top-5 right-0 whitespace-nowrap rounded bg-black px-2 py-0.5 text-[11px] italic',
